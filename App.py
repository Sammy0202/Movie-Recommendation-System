...
def get_movie_info(imdb_link):
    try:
        ...
        # Find the meta tag containing the movie description
        meta_tag = s_data.find("meta", {"name": "description"})
        if not meta_tag:
            return "Director info not found", "Cast info not found", "Story info not found"
        
        imdb_content = meta_tag["content"]
        
        # Split the content by ": Directed by " to separate director, cast, and story
        movie_info = imdb_content.split(": Directed by ")
        if len(movie_info) < 2:
            return "Director info not found", "Cast info not found", "Story info not found"
        director = "Director: " + movie_info[1].split(". With")[0].strip()
        # Extracting the cast information
        cast = "Cast: " + movie_info[1].split(". With ")[1].split(". ")[0].strip()
        # Extracting the story information
        story = "Story: " + movie_info[1].split(". With ")[1].split(". ")[1].split(". ")[0].strip()
        return director, cast, story
...
def KNN_Movie_Recommender(test_point, k):
    ...
    # Sort table based on ratings in descending order
...
def run():
    ...
    if cat_op == category[0]:
        ...
        if sel_gen:
                ...
                no_of_reco = st.slider('Number of movies you want Recommended:', min_value=5, max_value=20, step=1)
                test_point = [1 if genres in sel_gen else 0 for genres in genres]
                # test_point.append(imdb_s/core)
